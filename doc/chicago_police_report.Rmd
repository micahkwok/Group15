...
Simple and multivariable linear regression models were used to determine if there was an association between the number of complaints an officer receives per year and the officer's salary. Number of complaints, salary and officer demographic variables (i.e. gender, race and age) were the variables used from the original dataset. The R programming language [@R] and the following R packages: tidyverse [@tidyverse], doctopt [@docopt], knitr [@knitr], janitor [@janitor], lubridate [@lubridate], GGally [@GGally], broom [@broom] were used to conduct these analyses. The code used to perform the analysis and create this report can be found here: <https://github.com/UBC-MDS/CPD>

# Results and Discussion

To better answer whether a police staff member's salary influences their number of complaints per year, we found it helpful to start with a high level view of the Chicago Police Data and looked at the number of complaints per year.

```{r complaints images, echo = FALSE, out.width = '45%', out.height = '45%', fig.cap=c("Figure 1. Complaints per year (2005-2015)", "Figure 2. Police staff per year (2005-2015)"), fig.show='asis'}
knitr::include_graphics(c('../eda/images/complaints_by_year.png','../eda/images/staff_by_year.png'))
```

We see that the complaints per year have been falling from 2007 onwards in Figure 1, but a portion of it can be contributed to declining total staff numbers as seen in Figure 2. Instead, if we look at the complaints per year per officer in Figure 3, we see that the overall trend has not really changed from year to year.

```{r faceted histogram, echo = FALSE, out.width = '65%', out.height = '65%', fig.cap="Figure 3. Complaints per officer faceted by year"}
...
```{r officer breakdown image, echo = FALSE, out.width = '45%', out.height = '45%', fig.cap=c("Figure 4. Total complaints from 2005-2015", "Figure 5. Police Officer salary distribution from 2005-2015")}
knitr::include_graphics(c('../eda/images/complaints_by_rank.png','../eda/images/police_salary_dist.png'))
```

Taking a closer look at this Police Officer group, we found that their salary ranged from \$36,984 to \$122,100 with a mean of \$73,773 from the years 2005 to 2015. In terms of age, the average police officer in 2015 was about 41 years old and this was a relatively normal distribution. These officers had about 12 years of service but this distribution was harder to characterize with a number of officers having less than seven years of service.

```{r police officer distribution, echo = FALSE, out.width = '45%', out.height = '45%', fig.cap= "Figure 6. Distributions of Police Officer attributes in 2015", fig.show = 'hold', fig.align = 'center'}
...
```{r police officer demographics, echo = FALSE, out.width = '45%', out.height = '45%', fig.cap="Figure 7. Demographics of Police Officers in 2015", fig.show = 'hold', fig.align = 'center'}
...
```{r correlation matrix image, echo = FALSE, out.width = '75%', out.height ='75%', fig.cap="Figure 8. Correlation Matrix", out.width = "75%"}
...
```{r regression visualization, echo = FALSE, out.width = '75%', out.height = '75%', fig.cap="Figure 9. Scatterplot with Line of Regression in Blue", out.width = "75%"}
...
Nesvisky, M. (2007). Police Pay and Performance. Retrieved November 28, 2020, from https://www.nber.org/digest/jan07/police-pay-and-performance